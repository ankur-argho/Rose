...
function switchLanguage(lang) {
    ...
    elements.forEach(element => {
        if (lang === 'bn') {
            element.textContent = element.getAttribute('data-bn') || element.textContent;
        } else {
    ...
    if (lang === 'bn') {
        document.getElementById('recipient-name').placeholder = 'তার নাম লিখুন...';
        document.getElementById('sender-name').placeholder = 'আপনার নাম লিখুন...';
        document.getElementById('love-message').placeholder = 'আপনার হৃদয়ের বার্তা লিখুন...';
    } else {
        document.getElementById('recipient-name').placeholder = 'Enter their name...';
        document.getElementById('sender-name').placeholder = 'Enter your name...';
        document.getElementById('love-message').placeholder = 'Write your heartfelt message...';
    }

    // Update preview card if it exists
    updatePreviewCardLanguage(lang);
}

function updatePreviewCardLanguage(lang) {
    const previewName = document.getElementById('preview-name');
    const previewMessage = document.getElementById('preview-message');
    const previewSender = document.getElementById('preview-sender');

    if (previewName && previewName.textContent === 'Someone Special') {
        if (lang === 'bn') {
            previewName.textContent = 'কারও বিশেষজন';
        } else {
            previewName.textContent = 'Someone Special';
        }
    }

    if (previewMessage && previewMessage.textContent === 'Your beautiful message will appear here...') {
        if (lang === 'bn') {
            previewMessage.textContent = 'আপনার সুন্দর বার্তা এখানে প্রদর্শিত হবে...';
        } else {
            previewMessage.textContent = 'Your beautiful message will appear here...';
        }
    }

    if (previewSender && previewSender.textContent === 'You') {
        if (lang === 'bn') {
            previewSender.textContent = 'আপনি';
        } else {
            previewSender.textContent = 'You';
        }
    }
}
...
function initMessageForm() {
    ...
    const roseColors = {
        red: { class: 'rose-red', glow: 'rgba(233, 30, 99, 0.8)' },
        pink: { class: 'rose-pink', glow: 'rgba(240, 98, 146, 0.8)' },
    ...
    recipientInput.addEventListener('input', (e) => {
        previewName.textContent = e.target.value || (localStorage.getItem('preferredLanguage') === 'bn' ? 'কারও বিশেষজন' : 'Someone Special');
        previewName.style.animation = 'none';
        setTimeout(() => previewName.style.animation = '', 10);
    });

    senderInput.addEventListener('input', (e) => {
        previewSender.textContent = e.target.value || (localStorage.getItem('preferredLanguage') === 'bn' ? 'আপনি' : 'You');
    });

    messageInput.addEventListener('input', (e) => {
        previewMessage.textContent = e.target.value || (localStorage.getItem('preferredLanguage') === 'bn' ? 'আপনার সুন্দর বার্তা এখানে প্রদর্শিত হবে...' : 'Your beautiful message will appear here...');
    });
...
function showRoseCardModal(recipient, sender, message, rose) {
    ...
    const roseStyles = {
        red: { class: 'rose-red', glow: 'rgba(233, 30, 99, 0.4)' },
        pink: { class: 'rose-pink', glow: 'rgba(240, 98, 146, 0.4)' },
    ...
    // Determine language for the card
    const lang = localStorage.getItem('preferredLanguage') || 'en';
    const dearText = lang === 'bn' ? 'প্রিয়' : 'Dear';
    const withLoveText = lang === 'bn' ? 'ভালোবাসা নিয়ে,' : 'With love,';
    const roseDayText = lang === 'bn' ? 'গোলাপ দিবস' : 'Rose Day';
    ...
    finalCard.innerHTML = `
        ...
        <h3 style="font-family: 'Dancing Script', 'Noto Sans Bengali', cursive; font-size: 2.2rem; color: #ffb3cc; margin-bottom: 1.5rem; text-shadow: 0 0 20px rgba(255, 179, 204, 0.3);">${dearText} ${recipient}</h3>
        <p style="font-style: italic; color: rgba(255,255,255,0.85); line-height: 2; margin-bottom: 2rem; font-size: 1.1rem;">${message}</p>
        <div style="font-family: 'Dancing Script', 'Noto Sans Bengali', cursive; color: rgba(255,255,255,0.6);">
            ${withLoveText}<br>
        ...
        <div style="margin-top: 1.5rem; font-size: 0.9rem; color: rgba(255,255,255,0.4);">
            <span class="colored-rose rose-red">🌹</span> ${roseDayText} ${new Date().getFullYear()} <span class="colored-rose rose-red">🌹</span>
        </div>
    `;
